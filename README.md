...
- Загружается XLS/XLSX, ссылки берутся из первой колонки.
- Для `.xls` (старый формат) используется fallback-чтение первой колонки через pandas, чтобы избежать ошибки `BadZipFile`.
