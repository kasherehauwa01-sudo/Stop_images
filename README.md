...
1. На вход подается ссылка на раздел сайта (вручную через однострочное поле или через XLS/XLSX).
   - При загрузке XLS/XLSX приложение берет ссылки строго из первой колонки (поддерживаются обычные URL и Excel-гиперссылки/анкоры).
   - Для ссылок из первой колонки TinEye URL формируется напрямую без дополнительного сопоставления колонок.
...
## Вкладка «Проверка XLS»

- Загружается XLS/XLSX и в интерфейсе задается сопоставление колонок (из какой колонки брать ссылку, опционально артикул).
- Для `.xls` (старый формат) используется fallback-чтение через pandas/xlrd, чтобы избежать ошибки `BadZipFile`.
- Ссылки можно открыть в новых вкладках кнопкой `Открыть все ссылки в новых вкладках`.
- Anti-bot/captcha (галочка «Я не робот») подтверждается вручную пользователем в браузере.
